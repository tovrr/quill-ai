...
- [x] Executed a 2027 remediation v1 across the agent UX in recommended order: added shared UI settings model (`src/lib/ui-settings.ts`) and telemetry/variant utility (`src/lib/ui-experiments.ts`), wired dual conversation layout + focus mode + status surface + contextual actions + assistant avatar/bubble toggles into live `/agent` rendering, added composer behavior preferences (send-on-enter and action labels), enabled settings-driven compact thread spacing and auto-open-canvas behavior, and documented UX adoption constraints in `docs/patterns/CHAT_UI_REFERENCE_POLICY.md`
- [x] Implemented the next high-value thread UX fix: while streaming, Quill now surfaces explicit live status inside the message list (thinking vs streaming vs active tool execution) and lists currently running tool names, reducing invisible background activity during long tool runs
- [x] Applied a targeted assistant-ui-style adoption pass without changing Quill's backend: live tool-call states now render in-thread, tool invocations use expandable step cards with input/output details, artifact summaries can reopen Canvas directly from the message thread, and `TaskInput` now exposes active capability chips plus clearer composer hints
- [x] Completed Phase 6 shared-primitive adoption on core agent surfaces: `TaskInput` now uses shared `DropdownMenu` primitives for builder/mode pickers, and `CanvasPanel` now uses shared `Tabs` and `Badge` primitives for artifact switching and labeling with no typecheck regressions
- [x] Finished repo landscape review for chat-builder acceleration: `assistant-ui` is the strongest reusable UI primitive reference for Quill's current Vercel AI SDK + Next.js stack, `agno-agi/agent-ui` remains a good visual/product reference, `vercel/ai` should be consumed as a library/reference rather than forked, and `vstorm-co/full-stack-ai-agent-template` / `open-webui` are too opinionated and backend-heavy for direct adoption
- [x] Executed a 5-phase chat/builder UX bottleneck pass: smarter chat auto-scroll (no forced yank while reading), stream telemetry timings (`firstAssistantMs`, `firstRenderableMs`, `totalMs`), mobile action discoverability improvements in composer (web search/image/builder controls always visible), mobile overlay conflict guardrails (auto-close sidebar when canvas opens), and memoized `RealMessageBubble` to reduce long-thread rerenders
- [x] Added guest preview policy for React app Canvas rendering: `/api/preview` now supports configurable guest access with `API_PREVIEW_ALLOW_GUEST`, allows a limited number of free previews via `API_PREVIEW_GUEST_FREE_ATTEMPTS` + `API_PREVIEW_GUEST_WINDOW_MS`, and returns explicit auth-required errors after quota is exhausted
- [x] Improved Canvas preview UX for auth-gated guests: react-app preview tab remains available, failed preview responses now surface actionable messaging, and a direct "Sign in to preview" CTA is shown instead of silent fallback to code-only rendering
...
- [x] Distributed rate limiting: `src/lib/observability/rate-limit.ts` now uses Upstash Redis (INCR/EXPIRE/PTTL pipeline, 1500ms timeout) with automatic in-memory fallback; all API routes updated to `await checkRateLimit()`
...
- 2026-04-18: Continued next-order UI primitive cleanup and debt reduction pass: migrated raw interactive controls to shared primitives on `src/app/login/page.tsx`, `src/app/error.tsx`, `src/app/artifacts/page.tsx`, and `src/components/HeroInput.tsx` (Button/Input/Textarea adoption), and corrected artifacts list row structure to avoid nested interactive controls by splitting row-select and delete actions. Validation: `npm run typecheck` pass; UI standards scan now reports raw primitive count reduced from 32 to 16 with zero icon import violations.

- 2026-04-18: Executed recommended next-order UI consistency pass across extension surfaces: standardized interactive controls in `src/app/mcp/page.tsx`, `src/app/skills/page.tsx`, and `src/app/autopilot/page.tsx` to shared shadcn primitives (`Button`, `Input`, `Select`, `Textarea`), replaced remaining native auth-type select in MCP with shared Select, and removed raw icon-button title dependency on Autopilot actions by using explicit aria labels. Validation: `npm run typecheck` pass after each page migration.

- 2026-04-18: Completed phase-based UX optimization implementation on core chat surfaces: updated `src/app/agent/page.tsx` with bottom-stick-aware auto-scroll and stream completion telemetry, added sidebar/canvas mobile overlay conflict guardrails, adjusted `TaskInput` mobile toolbar visibility for key actions, and memoized `RealMessageBubble` for lower rerender cost in long conversations. Validation: `npm run typecheck` pass.

- 2026-04-18: Implemented guest preview quota and auth handoff for Canvas React previews: `/api/preview` now allows guest previews for a configurable limited count (default 2) and returns `preview_auth_required` on exhaustion; Canvas now surfaces the server error and renders a sign-in CTA in preview mode rather than failing silently. Validation: `npm run typecheck` pass and scripted API probe confirms attempt 1/2 return 200, attempt 3 returns 401 with auth-required payload.
...
- 2026-04-17: Completed remaining Phase 2 migrations and started Phase 4 validation closure: moved `src/lib/builder-artifacts.ts` -> `src/lib/builder/artifacts.ts`, `src/lib/api-metrics.ts` -> `src/lib/builder/metrics.ts`, `src/lib/api-security.ts` -> `src/lib/auth/security.ts`, `src/lib/entitlements.ts` -> `src/lib/auth/security.ts`, and `src/lib/openrouter-models.ts` -> `src/lib/models/openrouter.ts` with history-preserving renames and import rewiring; verified no stale legacy import paths remain. Validation results: `npm run typecheck` pass, `npm run build` pass, `npm run test:agent-remediation` pass, and `npm run test:execution` now executes with corrected import path but reports local Docker daemon unavailable.
