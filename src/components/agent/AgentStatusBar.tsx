...
export const AGENT_STATUS_MESSAGES: Record<AgentStatus, string> = {
  idle: "Ready",
  thinking: "Planning...",
  running: "Building...",
...
export function getAgentStatusMessage(status: AgentStatus): string {
  return AGENT_STATUS_MESSAGES[status];
}
...
export function AgentStatusBar({ status, taskTitle, stepCount, totalSteps, compact = false }: AgentStatusBarProps) {
  const color = getAgentStatusColor(status);
  const message = getAgentStatusMessage(status);
  const hasStepData = typeof stepCount === "number" && typeof totalSteps === "number" && totalSteps > 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <span
          className={`w-2 h-2 rounded-full ${status === "running" || status === "thinking" ? "animate-pulse" : ""}`}
      ...
      {!compact && taskTitle && (
        <>
          ...
          <span className="text-xs text-quill-muted truncate max-w-xs">{taskTitle}</span>
        </>
      )}
